...
authorization-related data in the :mod:`repoze.who` and :mod:`repoze.what`
``identity`` dictionaries.

.. warning::

    As of :mod:`repoze.what` v2, the groups and permissions will only be loaded
    in the :mod:`repoze.what` ``identity`` dictionary 
    (``environ['repoze.what.identity']``). So you are encouraged not to access
    this data from the :mod:`repoze.who` ``identity`` -- if you do, you will
    have to update your code when you want to upgrade to v2.
